(function( $ ){
  ...
  var _config_table=function(){
    defSettings.cellCount=defSettings._tableObj.children("tbody").find("tr:first td").length; //Count Cells
  ...
  var _set_row_cells_visibility=function(row){
      ...
      row.children("td:lt("+(defSettings.scrollInterval[1])+"):gt("+(defSettings.scrollInterval[0])+")").hide();
      row.children("td:eq("+defSettings.scrollInterval[0]+")").hide();
      row.children("td:eq("+defSettings.scrollInterval[1]+")").hide();

      var init_interval=(defSettings.scrollPosition*defSettings.intervalLength)+defSettings.scrollInterval[0]-(defSettings.intervalLength);
      var end_interval=(defSettings.scrollPosition*defSettings.intervalLength)+defSettings.scrollInterval[0]-1;
      row.children("td:lt("+end_interval+"):gt("+init_interval+")").show();
      row.children("td:eq("+end_interval+")").show();
      row.children("td:eq("+init_interval+")").show();        
  }
